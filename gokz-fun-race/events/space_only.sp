...
void SpaceOnly_OnClientDisconnect(int client)
{
    SpaceOnly_ResetStatus(client);
}

void SpaceOnly_OnTimerStart(int client)
{
    if(!SpaceOnly_CheckRaceType())
    {
        return;
    }
...
void SpaceOnly_OnTimerEnd(int client, float time, int cp)
{
    if(!SpaceOnly_CheckRaceType())
    {
        return;
  ...
  * @param jump 起跳详细数据
  */
void SpaceOnly_OnLanding(Jump jump)
{
    if(!SpaceOnly_CheckRaceType())
    {
        return;
